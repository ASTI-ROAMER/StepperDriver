...
void AbsStepper::setLimitSwPin(short pin_, long lim_abs_step, LimitSWPinSelect pin_select, short active_state, LimitSWMethod method){
    ...
    setHardLimAsPos(lim_abs_step, pin_select);
...
void AbsStepper::setLimitSwPin(short pin_, double jdeg, LimitSWPinSelect pin_select, short active_state, LimitSWMethod method){
    ...
    setLimitSwPin(pin_, convertJdeg2Pos(jdeg), pin_select, active_state, method);

}

void AbsStepper::setHardLimAsPos(long a_step, LimitSWPinSelect pin_select){
    if(pin_select == LIMIT_DOWN_PIN){
        if(limitsw_up_pin != SHRT_MIN){                     // there is a uppper limit switch value, so check it first before setting lower limit
            if(a_step < _lim_max_hard_pos){            // check if lower limit is lower than upper limit
                _lim_min_hard_pos = a_step;
        ...
        if(limitsw_down_pin != SHRT_MIN){                   // there is a lower limit switch value, so check it first before setting upper limit
            if(a_step > _lim_min_hard_pos){          // check if upper limit is higher than lower limit
                // Serial.println("Setting lim up to: " + String(a_step));
                _lim_max_hard_pos = a_step;
...
void AbsStepper::setHardLimUpPin(short pin_, long lim_abs_step, short active_state, LimitSWMethod method){
    // Serial.println("abs lim up: " + String(lim_abs_step));
    setLimitSwPin(pin_, lim_abs_step, LIMIT_UP_PIN, active_state, method);
}
void AbsStepper::setHardLimUpPin(short pin_, double jdeg, short active_state, LimitSWMethod method){
    // Serial.println("jdeg lim up: " + String(jdeg));
    setLimitSwPin(pin_, jdeg, LIMIT_UP_PIN, active_state, method);
}

void AbsStepper::setHardLimUpAsPos(long a_step){
    setHardLimAsPos(a_step, LIMIT_UP_PIN);
}
void AbsStepper::setHardLimUpAsAdeg(double ddeg){
    // setHardLimAsPos((long)(ddeg * step_per_deg), LIMIT_UP_PIN);
    setHardLimAsPos(convertAdeg2Pos(ddeg), LIMIT_UP_PIN);
}
void AbsStepper::setHardLimUpAsJdeg(double jdeg){
    // setHardLimAsPos((long)(jdeg * stepper2joint_ratio * step_per_deg), LIMIT_UP_PIN);
    setHardLimAsPos(convertJdeg2Pos(jdeg), LIMIT_UP_PIN);
}



void AbsStepper::setHardLimDownPin(short pin_, long lim_abs_step, short active_state, LimitSWMethod method){
    setLimitSwPin(pin_, lim_abs_step, LIMIT_DOWN_PIN, active_state, method);
}
void AbsStepper::setHardLimDownPin(short pin_, double jdeg, short active_state, LimitSWMethod method){
    setLimitSwPin(pin_, jdeg, LIMIT_DOWN_PIN, active_state, method);
}

void AbsStepper::setHardLimDownAsPos(long a_step){
    setHardLimAsPos(a_step, LIMIT_DOWN_PIN);
}
void AbsStepper::setHardLimSWDownAsAdeg(double ddeg){
    // setHardLimAsPos((long)(ddeg * step_per_deg), LIMIT_DOWN_PIN);
    setHardLimAsPos(convertAdeg2Pos(ddeg), LIMIT_DOWN_PIN);
}
void AbsStepper::setHardLimDownAsJdeg(double jdeg){
    // setHardLimAsPos((long)(jdeg * stepper2joint_ratio * step_per_deg), LIMIT_DOWN_PIN);
    setHardLimAsPos(convertJdeg2Pos(jdeg), LIMIT_DOWN_PIN);
}
...
void AbsStepper::setCurPosAtAdeg(double abs_deg){
    // setCurPosAtPos(long(abs_deg * step_per_deg));
    setCurPosAtPos(convertAdeg2Pos(abs_deg));
}

void AbsStepper::setCurPosAtJdeg(double jdeg){
    // setCurPosAtPos(long(jdeg * stepper2joint_ratio * step_per_deg));
...
void AbsStepper::setSoftLimAsJdeg(double min, double max, uint8_t use_soft_limits_){
    setSoftLimAsPos(convertJdeg2Adeg(min), convertJdeg2Adeg(max), use_soft_limits_);
}
...
void AbsStepper::printStats(){
    Serial.println("\n****************"
                   ...
                   "\nlimitsw_down_reset_astep: " + String(_lim_min_hard_pos) +
                   "\nlimitsw_up_reset_astep: " + String(_lim_max_hard_pos) +
                   "\n_min_soft_astep: " + String(_min_soft_pos) +
                   "\n_max_soft_astep: " + String(_max_soft_pos) +
                   "\n****************\n"
                   );
}